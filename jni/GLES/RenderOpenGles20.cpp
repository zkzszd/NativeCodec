...
RenderOpenGles20::RenderOpenGles20(int angle):
_id(0),
 _textureWidth(-1),
 _textureHeight(-1)
{
	...
	{
	case 0:
		memcpy(_vertices, vertices[0], sizeof(_vertices));
		break;
	case 90:
		memcpy(_vertices, vertices[1], sizeof(_vertices));
		break;
	case 180:
		memcpy(_vertices, vertices[2], sizeof(_vertices));
		break;
	case 270:
		memcpy(_vertices, vertices[3], sizeof(_vertices));
		break;
	case -90:
		memcpy(_vertices, vertices[4], sizeof(_vertices));
		break;
	default:
		memcpy(_vertices, vertices[0], sizeof(_vertices));
		break;
...
{
    WEBRTC_TRACE(kTraceDebug, kTraceVideoRenderer, _id, "%s: id %d",__FUNCTION__, (int) _id);
