...
typedef bool(*pf_AMediaFormat_getBuffer)(AMediaFormat*, const char *name, void** data, size_t *size);
...
const char* AMEDIAFORMAT_KEY_I_FRAME_INTERVAL = "i-frame-interval";		//I帧间隔
...
init_symbols()
{
	...
	char versions[8];
	if (__system_property_get("ro.build.version.sdk", versions) >= 0)
	{
		g_sdkVersion = atoi(versions);
	}
	...
	LOGE("sdk version %d",g_sdkVersion);
	...
	for (int i = 0; members[i].name; i++)
	{
		...
		{
			LOGW("con not find simbol %s", members[i].name);
			dlclose(ndk_handle);
			goto end;
		}
	...
	ret = i_init_state == 1;
	if (!ret)
		LOGE("MediaCodec NDK init failed");
